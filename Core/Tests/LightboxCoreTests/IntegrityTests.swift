...
struct IntegrityTests {
    ...
    @Test func aGarbledHeaderFailsToOpenRatherThanBeingAcceptedSilently() throws {
        ...
        //
        // `IndexStore.close()` used to not be enough to prevent it: GRDB's
        ...
        // and SQLite's automatic checkpoint-on-last-close never ran at all
        // once a reader connection existed. That is #40 — `close()` now runs
        // `Database.checkpoint(.truncate)` on the writer itself before
        // `pool.close()`, folding every frame back into `index.sqlite` and
        // resetting `-wal` to empty, so the explicit `testExecute` checkpoint
        // this test used to need here is gone; `close()` alone does the job.
        // The sidecars are still removed outright afterward (as
        // `IndexStore.rebuild(at:)` does) so "gone" is verifiable by
        // existence, not by trusting the checkpoint left nothing readable.
        try first.close()
        ...
        // The checkpoint `close()` now runs is load-bearing, not incidental
        // cleanup: without it, everything the migrator wrote is still sitting
        // in the (now-removed) `-wal`, and `index.sqlite` on disk is a bare
        // 4096-byte page-1 stub — a valid header over no schema. A garbled
        // version of that stub would still fail to open, so the detection
        // assertions below would pass whether or not the checkpoint ran,
        // proving nothing. This assertion pins the checkpoint as the thing
        // that put the schema where the garble can actually reach it.
        // (Verified by mutation: strip the checkpoint out of `close()` but
        // keep the sidecar removals here, and this goes red — the file is
        // exactly 4096 bytes.)
